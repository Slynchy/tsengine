...
export class GameObject {
  ...
  private fireEvent(key: string, ...params: any[]): void {
    // @ts-ignore
    for(const ev of (this[key]) as { [key: string]: Function; }) {
      ev(params);
    }
  }
  ...
  public removeComponent(_component: typeof Component): void {
    if(!this.hasComponent(_component)) throw new Error("Could not remove component from GameObject; doesn't exist!");
